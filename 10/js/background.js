class ParallaxBackground {
    constructor(camera) {
        this.layers = [];
        [{image: "layer1", speed: 0, scale:2.5, position:new Vector2(0,0)},      
         {image: "layer2", speed: 5, scale:2.5, position:new Vector2(0,200)}    
        ].forEach(element => {
        ...
        this.layers.forEach(layer => {
            if(layer.speed === 0){
                layer.positions.forEach(p=>{
                    p.x =  this.camera.globalPosition.x;
                    p.y =  this.camera.globalPosition.y;
                })
            }
            else{
                const offset = new Vector2(distanceToMove.x * layer.speed * deltaTime, distanceToMove.y * layer.speed * deltaTime);
                layer.positions.forEach(p=>{
                    p.subEqual(offset);
                })
